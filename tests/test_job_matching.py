...
def test_wanted_employment_type_is_required():
    job = RawJob(
        title="Data Analyst",
...
def test_profile_can_select_full_time():
    job = RawJob(
        title="Data Analyst",
        company="Example GmbH",
        location="Berlin",
        description="Wir suchen eine Data Analystin in Vollzeit mit Python und SQL.",
        url="https://careers.example.com/jobs/124"
    )
    score, _ = score_job(job, profile(employment_types=["Vollzeit"]))
    assert score >= 60
